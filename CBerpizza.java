...
static class Customer implements Comparable<Customer>{
    int id;
    int m;
    public Customer(int id,int m){
        this.id=id;
        this.m=m;
    }
    @Override
    public int compareTo(Customer o){
        if(this.m!=o.m)return Integer.compare(o.m,this.m);
    ...
    static void solve(){
        int q=sc.nextInt();
    int id=1,p=1;
    boolean[]vis=new boolean[q+1];
    PriorityQueue<Customer>pq=new PriorityQueue<>();
    for(int i=0;i<q;i++){
        int ty=sc.nextInt();
        if(ty==1){
            int m=sc.nextInt();
            pq.add(new Customer(id,m));
            id++;
        }else if(ty==2){
            while(p<id&&vis[p])p++;
            vis[p]=true;
            ...
            while(!pq.isEmpty()&&vis[pq.peek().id])pq.poll();
            Customer b=pq.poll();
            vis[b.id]=true;
            out.print(b.id+" ");
        }
