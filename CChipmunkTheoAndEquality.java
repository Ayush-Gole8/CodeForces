...
public class CChipmunkTheoAndEquality {
    ...
    static void printf(String fmt, Object... args) { out.printf(fmt, args); }
    ...
    static long add(long a, long b, long mod) { return (a % mod + b % mod + mod) % mod; }
    static long sub(long a, long b, long mod) { return ((a - b) % mod + mod) % mod; }
    ...
    // [BITWISE]
    // ─────────────────────────────────────────────────────────
    static int popcount(long x)  { return Long.bitCount(x); }
    static int msb(long x)       { return 63 - Long.numberOfLeadingZeros(x); }  // floor(log2(x))
    static long nextPow2(long x) { return x <= 1 ? 1 : Long.highestOneBit(x - 1) << 1; }
    ...
    static List<long[]>[] buildWeightedGraph(int n, int m) {
        ...
        for (int i = 0; i < m; i++) {
            ...
            g[u].add(new long[]{v, w}); g[v].add(new long[]{u, w});
        }
        return g;
    }
    ...
    static class SegTree {
        ...
        long op(long a, long b) { return a + b; }  // sum segment tree
        long identity()         { return 0L; }
    ...
    static long binarySearchLast(long lo, long hi, java.util.function.LongPredicate check) {
        while (lo < hi) {
            ...
            if (check.test(mid)) lo = mid; else hi = mid - 1;
        }
    ...
    static long binarySearchFirst(long lo, long hi, java.util.function.LongPredicate check) {
        while (lo < hi) {
            long mid = lo + (hi - lo) / 2;
            if (check.test(mid)) hi = mid; else lo = mid + 1;
        }
        return check.test(lo) ? lo : lo + 1;
    }
    ...
    /** All sting indices of pattern in text (0-indexed). */
    static List<Integer> kmpSearch(String text, String pattern) {
        ...
        for (int i = pl + 1; i < s.length(); i++)
            if (f[i] == pl) positions.add(i - 2 * pl);
        return positions;
    }
    ...
    static void printArr(int[] a)  { for (int x : a) out.print(x + " "); out.println(); }
    static void printArr(long[] a) { for (long x : a) out.print(x + " "); out.println(); }

    static long[] toLong(int[] a) { long[] r = new long[a.length]; for (int i = 0; i < a.length; i++) r[i] = a[i]; return r; }
...
static void solve() {
    ...
    for (int st : arr) {
        ...
        int steps=0;
        while (true) {
            mpp.merge(x,(long)steps,Long::sum);
            mpp2.merge(x, 1, Integer::sum);
            if (x==1) break;
            if ((x&1)==1) x++;
            else x/=2;
            steps++;
    ...
    for (Map.Entry<Integer, Long> e : mpp.entrySet()) {
        if (mpp2.get(e.getKey()) == n) {
    ...
    static class FastScanner {
        ...
        private int skipWS() { int c = read(); while (c <= 32) c = read(); return c; }
        ...
        long nextLong() {
            int c = skipWS(); int sgn = 1; long val = 0;
            if (c == '-') { sgn = -1; c = read(); }
            while (c > 32) { val = val * 10 + (c - '0'); c = read(); }
            return val * sgn;
        }
        ...
        long[] nextLongArray(int n) { long[] a = new long[n]; for (int i = 0; i < n; i++) a[i] = nextLong(); return a; }

        int[][] nextIntMatrix(int r, int c) {
            int[][] m = new int[r][c];
            for (int i = 0; i < r; i++) for (int j = 0; j < c; j++) m[i][j] = nextInt();
            return m;
        }
    }
}